...
> **Live Application:** [https://return-desk-frido.vercel.app/](https://return-desk-frido.vercel.app/)  
...
- **Query Layer:** `pg` (node-postgres) with parameterized SQL queries (no ORM)
- **Validation:** Zod 4 (Strict schemas for API request validation and frontend forms)
- **Styling:** Tailwind CSS v4 with custom token system
...
- **Node.js:** version 20.6+ or later (required for Node native `--env-file` support)
- **PostgreSQL:** version 14.x or later (local PostgreSQL instance, Docker container, or cloud instance such as Neon)
...
# 5. Seed sample data (populates 36 requests across all statuses and reasons)
npm run db:seed
...
_For full column definitions, indexes, and constraint statements, see [docs/DATABASE.md](docs/DATABASE.md)._
...
_For complete request bodies, error envelopes, and curl examples, see [docs/API.md](docs/API.md)._
...
_For source code symbol traceability, see [docs/CODEMAP.md](docs/CODEMAP.md)._

---

## 6. Key Design Decisions & Alternatives Considered

### 1. Parameterized SQL over an ORM (Prisma / Drizzle)

- **Decision:** All database interactions use `pg` with parameterized SQL statements (`$1`, `$2`, ...).
- **Rationale:** The brief explicitly states: _"Loading every row into the browser and filtering it there is not acceptable, and we will look at your queries."_ Hand-written SQL provides direct visibility into query execution plans, avoids accidental N+1 queries, and allows precise row-level locking (`SELECT ... FOR UPDATE`).
...
- **Rationale:** The frontend never decides what is legal; it simply renders the buttons the server permits. This ensures UI and API rules stay synchronized without duplicate logic.
...
| **A13** | Search matching                    | Case-insensitive partial match (`ILIKE`) across reference, order number, customer name, and email. | Matches the brief requirement _"search by customer, order or reference"_.                                                   |
...
ReturnDesk implements a clean, tactile interface with three interaction tiers:

- **Tier 1 (Quiet):** Table rows, notes, pagination links. Shifts background to hover state and reveals an ink accent on the left edge (`hover:before:bg-ink`).
- **Tier 2 (Controls):** Buttons, filter chips, selects. Features tactile click feedback via `@utility pressable`.
- **Tier 3 (Tiles):** Mobile request cards, resolution choice tiles, return reason tiles. Lifts slightly on hover (`translate(-1px, -1px)` with `shadow-lift`), and presses flat on tap.
- **Brand Palette:** Warm neutral off-white canvas (`#F7F7F5`), crisp paper white (`#FFFFFF`), deep ink (`#101820`), and Frido yellow accent (`#FCD00B`) for references and active lifecycle indicators.
- **Design Tokens:** All component styles derive from Tailwind semantic tokens.
- **Accessibility:** Full keyboard navigability (Tab, Enter, Space, Arrow keys for ChoiceTiles), visible `:focus-visible` rings on all interactive controls, and `prefers-reduced-motion` overrides.
...
1. **The Returns Desk (`/`):**
   ...
   - Type `"Ritika"` into search: observe the 300ms debounce firing a network request and filtering down to Ritika Bansal.
   - Toggle status filter chips (e.g. `In review`): URL query string updates automatically, and pagination resets to page 1.
   - Switch viewport to 375px mobile view: table transitions to stacked cards with zero horizontal overflow.
2. **Request Detail (`/requests/RD-00010`):**
   ...
   - Click **Approve…**:
     ...
     - Observe the server validation refusal rendered inside the modal: _"Refund amount is required"_.
...
4. **Soft Removal (`RD-00005`):**
   ...
   - Redirects to `/` with confirmation toast: _"RD-00005 was removed from the desk."_
   - Navigating directly to `/requests/RD-00005` displays the clean _"Request not found."_ empty state.
...
- **Phase 1: Database Schema & Lifecycle State Machine**
  ...
  - Deterministic seed script generating 36 requests covering status × reason combinations (`db/seed.ts`).
  - Domain constants, string unions, and pure state machine (`lifecycle.ts`).
...
- **Phase 3: Frontend Desk, Detail & Forms**
  - Token system and utilities in `src/app/globals.css`.
  - Base UI components in `src/components/ui/` with 3 interaction tiers.
  ...
  - Responsive design (375px layout) and accessibility support (keyboard navigation, focus states, ARIA labels).
- **Phase 4: Production Deployment & Hardening**
  - Live deployment hosted on Vercel: [https://return-desk-frido.vercel.app/](https://return-desk-frido.vercel.app/)
  - Database connectivity error handling (mapping network drops/timeouts and pool exhaustion to `503 SERVICE_UNAVAILABLE`).
  - Optional catch-all API route (`/api/[[...slug]]`) returning structured JSON 404 for unknown API endpoints and `/api` root.
  - Database seed data sanitization with invented customer and plain agent names.
...
## 12. Time Spent & Build Log

Approximate breakdown across the 24-hour take-home window:

- **Hours 1–3 (Phase 0 & Phase 1 — Schema, Core & Data):**
  - Neon PostgreSQL setup and `001_init.sql` migration (tables, check constraints, partial unique index, trigger).
  - Domain constants (`constants.ts`) and pure status lifecycle machine (`lifecycle.ts`).
  - Strict Zod validation schemas (`schemas.ts`) and API response/error helpers (`respond.ts`, `errors.ts`).
  - Deterministic 36-request seed script (`db/seed.ts`) covering all 25 visible status × reason pairs.
- **Hours 4–7 (Phase 2 — REST API & Documentation):**
  - Query layer with parameterized SQL (`src/lib/queries/`).
  - Service layer with transactional row locking and business rule validations (`src/lib/services/`).
  - Route handlers for `/api/requests`, `/api/requests/:reference`, `/transitions`, `/notes`, and optional catch-all 404.
  - Comprehensive reference docs (`docs/API.md`, `docs/DATABASE.md`, `docs/CODEMAP.md`).
- **Hours 8–13 (Phase 3 — Frontend Desk, Details & Forms):**
  - Design token system and theme setup in `src/app/globals.css`.
  - Base UI components (Button, Input, Select, Textarea, Modal, Toast, Banner, Badge, Skeleton, ChoiceTile).
  - Returns Desk (`/`): debounced search, status/reason filter chips, sort options, responsive table and cards, pagination.
  - Request Detail (`/requests/[reference]`): lifecycle progress bar, request details card, notes timeline, action modals for transitions.
  - Create and edit forms (`/requests/new`, `/requests/[reference]/edit`) with shared validation and server rule alignment.
  - Mobile responsiveness (375px layout) and accessibility review (keyboard navigation, focus states, ARIA labels).
- **Hours 14–15 (Phase 4 — Deployment & Hardening):**
  - Production deployment to Vercel connected to Neon PostgreSQL ([https://return-desk-frido.vercel.app/](https://return-desk-frido.vercel.app/)).
  - Database connectivity error handling (mapping connection timeouts/drops to 503 Service Unavailable).
  - Seed data sanitization, schema boundary tightening, and edge case fixes.
