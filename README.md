...
| Find a request       | Search by customer name or email, order number or reference; filter by status and reason; sort; page. All of it runs in one SQL query on the server. The desk state lives in the URL, so refresh, back and shared links keep the view.                                  |
